...
!!!annual improvements in new and existing shells
      ExistImprv = ExistImprv * 0.4
      NewImprv = NewImprv * 0.1
...
!!!PH: change the shell heating and cooling factors
          ShellHeatFactor(IBLDTP,IREG,2,CBECSyear-1989) = ShellHeatFactor(IBLDTP,IREG,2,CBECSyear-1989) * 0.1
          ShellCoolFactor(IBLDTP,IREG,2,CBECSyear-1989) = ShellCoolFactor(IBLDTP,IREG,2,CBECSyear-1989) * 0.1
...
!!!PH: estimate steam EUI
      DO r= 1, MNUMCR-2
       DO b= 1, CMnumBldg
         DistServSteamEUI (r,b,3) = DistServSteamEUI (r,b,3) * 0.5
       END DO ! b
      END DO ! r
...
!!!PH: update district service efficiency
      DO f= 1, CMnumMajFl
        DistServBoilerEff (f) = DistServBoilerEff (f) * 0.5
      END DO
!
      IF(PRTDBGK.EQ.1) Then
        WRITE(RCDBG,*) NumErr,' errors detected.'
