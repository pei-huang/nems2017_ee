...
!!!PH: adjust the heating and cooling shell efficiencies
        HTSHEFF(I) = HTSHEFF(I) * 0.1
        CLSHEFF(I) = CLSHEFF(I) * 0.1
...
!!!PH: loosen the shell efficiency improvement limit (0.3 -> 0.2)
       LIMIT=0.05  !Limits shell improvements to 70% over base year, applies to existing, new, heating, cooling
...
!!! TECHG is a parameter that represents the annual increase in existing shell integrity due to technology improvements
      DO Y1=RECSYEAR+1,LASTYR+baseyr-1
        DO B=1,MNUMBLDG
         TECHG(Y1,1,B)=TECHG(Y1-1,1,B)+0.005 + 0.025
         TECHG(Y1,2,B)=TECHG(Y1-1,2,B)+0.003 + 0.025
         TECHG(Y1,3,B)=TECHG(Y1-1,3,B)+0.006 + 0.025
         TECHG(Y1,4,B)=TECHG(Y1-1,4,B)+0.003 + 0.025
         TECHG(Y1,5,B)=TECHG(Y1-1,5,B)+0.006 + 0.025
         TECHG(Y1,6,B)=TECHG(Y1-1,6,B)+0.016 + 0.025
         TECHG(Y1,7,B)=TECHG(Y1-1,7,B)+0.003 + 0.025
         TECHG(Y1,8,B)=TECHG(Y1-1,8,B)+0.003 + 0.025
         TECHG(Y1,9,B)=TECHG(Y1-1,9,B)+0.011 + 0.025
        END DO
      END DO
